...
if __name__ == "__main__":
    ...
    case_folder_absolute_path = data_folder_absolute_path + '/' + test_case
    test_path = []
    
    
    
    full_file_names = [f for f in os.listdir(case_folder_absolute_path) if not f.endswith(('.yaml', '.md'))]
    if test_case == 'case_old':
        full_file_names = old_case_file_names
    file_names = [name for name in full_file_names]
    # print(file_names)
    for name in file_names:
        print(name)
    file_name = file_names[test_id]
    test_path = [test_case, case_folder_absolute_path, file_names[test_id], test_id]


    print(f"test_path:{test_path}")

    case_name, case_folder_absolute_path, file_name, index = test_path
    ...
    folder_name = 'EPSILON/' + 'test'
    # folder_name = 'EPSILON_0918_fix_egopath'
    
    # start_frame = 150
    
    print("params: {case_name}, {start_frame}, {init_angle}, {use_ego_path}")
    print(f"params: {case_name}, {start_frame}, {init_angle}, {use_ego_path}")
